...
# Order schema changes are intentionally manual. The service does not apply
...
try {
    ...
    $infrastructureProject = Join-Path $repoRoot 'src\Services\Order\Ecommerce.OrderService.Infrastructure\Ecommerce.OrderService.Infrastructure.csproj'
    $startupProject = Join-Path $repoRoot 'src\Services\Order\Ecommerce.OrderService.Api\Ecommerce.OrderService.Api.csproj'
    $migrationsDirectory = Join-Path $repoRoot 'src\Services\Order\Ecommerce.OrderService.Infrastructure\Migrations'
    ...
    $versionPattern = '^\d{14}_order_(?<version>\d{3})\.cs$'
    $versions =
    ...
    $migrationName = 'order_{0}' -f $nextVersion.ToString('000')
    $arguments = @(
        'ef'
        'migrations'
        'add'
