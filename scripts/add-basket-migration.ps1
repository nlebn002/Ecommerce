...
try {
    ...
    $infrastructureProject = Join-Path $repoRoot 'src\Services\Basket\Ecommerce.BasketService.Infrastructure\Ecommerce.BasketService.Infrastructure.csproj'
    $startupProject = Join-Path $repoRoot 'src\Services\Basket\Ecommerce.BasketService.Api\Ecommerce.BasketService.Api.csproj'
    $migrationsDirectory = Join-Path $repoRoot 'src\Services\Basket\Ecommerce.BasketService.Infrastructure\Migrations'
    ...
    $versionPattern = '^\d{14}_basket_(?<version>\d{3})\.cs$'
    $versions =
    ...
    $migrationName = 'basket_{0}' -f $nextVersion.ToString('000')
    $arguments = @(
        'ef'
        'migrations'
        'add'
